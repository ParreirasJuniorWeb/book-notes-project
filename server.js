...
app.get("/edit/:id", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/api/books/${req.params.id}`, 
      await withAuth({})
    );
    ...
    console.error("❌ Erro ao carregar livro:", error.response?.data || error.message);
    res.render("error.ejs", { 
...
app.post("/books", async (req, res) => {
  ...
  try {
    await axios.post(`${API_URL}/api/books`, req.body, 
      await withAuth({})
    );
    res.redirect("/dashboard");
  } catch (error) {
...
app.post("/books/:id/update", async (req, res) => {
  ...
  try {
    await axios.put(`${API_URL}/api/books/${req.params.id}`, req.body, 
      await withAuth({})
    );
    res.redirect("/dashboard");
  } catch (error) {
...
app.post("/books/:id/delete", async (req, res) => {
  try {
    await axios.delete(`${API_URL}/api/books/${req.params.id}`, 
      await withAuth({})
    );
    res.redirect("/dashboard");
  } catch (error) {
